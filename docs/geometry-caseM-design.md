...
- **採用確定(CP-M2 実装時判明方式の記録、`760ddeb`・2026-06-11 目視判定)**: 星状・羊歯とも上記初期パラメタを**そのまま採用**。12 枚判定で、羊歯の 5 対が樹枝状の 3 対と一目で判別できること・最内対(offsetZ 0.3)が設計どおりハブから出ること・腕間の接触がないことを確認。
...
- **採用確定(CP-M2 実装時判明方式の記録、`760ddeb`・2026-06-11 目視判定)**: **案 1(R 0.25 / H 2.0)を採用、案 2/3 のスイープは省略**。角柱(H/D 1.875)比で一目区別でき、針との紛れなし(針は 12 本クラスタのゲシュタルトで別物)を確認。
...
- **目視判定済み(2026-06-11、スクショ 12 枚全合格)**: 3 定番とも分類・描画・マーカー追従を確認。P1d/P1e 境界はスライダー 0.230 ⇔ 0.250 で形態が切り替わることを確認(デジタイズ値 sTop 1.12 どおり)。
...
- **CP-M2 完了記録(`760ddeb`)**: テスト 79 → 89。d.ts diff = union 3 値追加の 1 行のみ → **基線 #3: dist `2e7a1f659447357ed0ad9cf4a4a5839c` / d.ts `68a07ca8a7a574a2a140a603e3d4ec38`**(以後この基線で照合を再開)。ビット不変はフルメッシュ MD5 署名(頂点 + インデックス + ワールド行列、既存 11 形態 × 2 seed = 22 署名)の変更前後一致で機械証明し、**チャット側 Linux でも a511a8d ⇔ 760ddeb 間で 22 署名一致 — クロスマシン再証明済み**。
